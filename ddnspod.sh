...
# Common Params. This parameter is not required for this API. (optional)
region=""
service="dnspod"
version="2021-03-23"
...
query_internet_ip() {
  ...
  if [ -z "${ns_ip_addr}" ]; then
    logger -p error -s -t "${LOG_TAG}" "${domain_full_name} ${rec_type} [nslookup]: IP address extraction failed"
    return 1
  fi
}
...
query_local_ip() {
  ...
  # Validate IP address
  if [ -z "${ip_addr}" ]; then
    logger -p error -s -t "${LOG_TAG}" "Failed attempt to retrieve ${ip_version} address for ${domain_full_name}"
    return 1
  fi
...
# Function to send a request to Tencent Cloud DNSPod API 3.0 services
...
sync_ddns_rec() {
  ...
  if [ -z "${domain}" ]; then
    logger -p error -s -t "${LOG_TAG}" "Missing required field 'domain' in the config file '${CONF_FILE}', check it out."
    return 1
  fi

  # Validate network interface
  if ! ip link ls dev "${interface}" >/dev/null 2>&1 ; then
    [ "${LOG_LEVEL}" -lt "${LOG_LEVEL_ERROR}" ] || echo "Error: Invalid interface '${interface}' or unavailable"
    return 1
  fi
  ...
  if [ "${ip_addr}" = "${ns_ip_addr}" ]; then
    [ "${LOG_LEVEL}" -ne "${LOG_LEVEL_VERBOSE}" ] || logger -p info -s -t "${LOG_TAG}" "${domain_full_name} ${ip_version} address ${ip_addr} is up to date"
  ...
  if [ -z "${record_id}" ] || [ -z "${record_ip}" ]; then
    logger -p error -s -t "${LOG_TAG}" "Fail attempt to extract RecordId or IP address for ${domain_full_name} ${rec_type} from DNSPod API response"
    return 1
  fi
...
validate_config() {
  ...
  if [ ! -f "${CONF_FILE}" ]; then
    logger -p error -s -t "${LOG_TAG}" "Config file '${CONF_FILE}' not found."
    return 1
  fi

  # Exit if no DDNS records are found in the config file.
  if ! grep -q "DDNS" "${CONF_FILE}" ; then
    logger -p warn -s -t "${LOG_TAG}" "No DDNS records in '${CONF_FILE}'."
    return 1
  fi

  # Validate the log level
  if ! echo "${LOG_LEVEL}" | grep -q "^[01]$" ; then
    logger -p error -s -t "${LOG_TAG}" "Error: Invalid log level '${LOG_LEVEL}', Use 0 (Error) or 1 (Verbose)"
    return 1
  fi
  ...
  if [ -z "${secret_id}" ] || [ -z "${secret_key}" ]; then
    logger -p error -s -t "${LOG_TAG}" "API credentials fields 'SecretId' or 'SecretKey' are missing in '${CONF_FILE}'."
    return 1
  fi
  ...
  if [ -z "${log_file}" ]; then
    logger -p warn -s -t "${LOG_TAG}" "Log file field 'LogFile' missed in in '${CONF_FILE}'."
  else
    LOG_FILE="${log_file}"
  fi
}
